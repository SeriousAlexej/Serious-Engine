...
CTerrainPtr _ptrLastEditedTerrain;
...
static std::vector<std::unique_ptr<CTIButton>> dcButtons;
...
{
  pToolTipText+= sprintf(pToolTipText, "%s", static_cast<const char*>(_strToolTip));
}
...
void LoadSurface(CTFileName fnSurface)
{
  ...
  try
  {
    ...
    for( INDEX iLayer=0; iLayer<ctLayers; iLayer++)
    {
      ...
      CTerrainLayerPtr ptlLayer=ptrTerrain->tr_atlLayers[0];
      if( iLayer!=0)
      {
...
void SaveSurface(CTFileName fnSurface)
{
  ...
  try
  {
    ...
    for( INDEX iLayer=0; iLayer<ctLayers; iLayer++)
    {
      ...
      CTFileName fnTexture=CTextureDataPtr(ptlLayer->tl_ptdTexture)->GetName();
...
void PointToIconSpace(CPoint &pt, CTIButton *ptib)
{
  ...
  pt.x=Clamp<INDEX>(pt.x-x, INDEX(0), INDEX(ptib->tib_fdx));
  pt.y=Clamp<INDEX>(pt.y-y, INDEX(0), INDEX(ptib->tib_fdy));
}

void PointToScreenSpace(CPoint &pt, CDrawPortPtr pdp)
{
  HWND hWnd=CViewPortPtr(pdp->dp_Raster().ra_pvpViewPort)->vp_hWnd;
...
void GenerateNonExistingTerrainEditBrushes(void)
{
  for(INDEX iBrush=0; iBrush<CT_BRUSHES; iBrush++)
  {
    ...
    else
    {
      ...
      try
      {
        _pTextureStock_Obtain_t(fnBrush);
      }
      catch(char *strError)
      {
        (void) strError;
...
void CTerrainInterface::OnPaint() 
{
  ...
  {
    if(dcButtons.empty() || GetTerrain()!=_ptrLastEditedTerrain)
    {
...
void CTerrainInterface::RenderInterface(CDrawPortPtr pdp) 
{
#if ALLOW_TERRAINS
  // render terrain interface buttons
  for (auto& ittib : dcButtons)
  {
    CTIButton &tib=*ittib;
    PIX pixOffsetX=0;
    PIX pixOffsetY=0;
...
void GenerateTerrainBrushTexture( INDEX iBrush, FLOAT fHotSpot, FLOAT fFallOff)
{
  ...
  ii.ii_Picture=(UBYTE*) AllocMemory_(ii.ii_Width*ii.ii_Height*ii.ii_BitsPerPixel/8);
...
CTIButton *AddButton(std::vector<std::unique_ptr<CTIButton>> &dc, FLOAT x, FLOAT y, FLOAT dx, FLOAT dy, INDEX iLayer, INDEX iIcon,
                     CTString strToolTip, COLOR colBorder=C_RED|CT_TRANSPARENT, COLOR colFill=C_BLACK|CT_TRANSPARENT)
{
  auto ptib = std::make_unique<CTIButton>();
  auto& tib = *ptib;
  tib.tib_fx=x;
  tib.tib_fy=y;
  tib.tib_fdx=dx;
...
void CTIButton::SetFunctions(
  ...
  void (*pOnRightClickMove)(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp)/*=NULL*/,
...
void RenderBrushNo(CTIButton *ptib, CDrawPortPtr pdp)
{
...
void RenderBrushShape( INDEX iBrush, PIXaabbox2D rect, CDrawPortPtr pdp)
{
  CDrawPort dpBrush(pdp, rect);
  CTextureObject to;
  CTString strBrushFile;
  strBrushFile.PrintF("Textures\\Editor\\TerrainBrush%02d.tex", iBrush);
  try
...
void RenderBrushShape(CTIButton *ptib, CDrawPortPtr pdp)
{
...
void RenderBrushPressure(CTIButton *ptib, CDrawPortPtr pdp)
{
  pdp->SetFont( _pfdConsoleFont);
  pdp->SetTextAspect( 1.0f);
  pdp->SetTextScaling( 1.0f);
...
void RenderLayerTexture(CTIButton *ptib, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  CTerrainLayerPtr ptlLayer=GetLayer(iLayer);
  if(ptlLayer && ptlLayer->tl_ptdTexture!=NULL)
  {
    CTextureObject to;
    CTextureDataPtr ptd(ptlLayer->tl_ptdTexture);
    to.SetData(*ptd);
...
void RenderLayerBcg(CTIButton *ptib, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
...
void RenderLayerMask(CTIButton *ptib, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  CTerrainLayerPtr ptlLayer=GetLayer(iLayer);
  if(ptlLayer && ptlLayer->tl_ptdTexture!=NULL)
  {
    CTextureObject to;
    CTextureDataPtr ptd=ptlLayer->GetThumbnail(64,64);
    to.SetData(*ptd);
    PIXaabbox2D boxScreen=GetButtonScreenBox(*ptib);
    pdp->PutTexture( to, boxScreen);
  }
...
void UpdateEditModeIcon(CTIButton *ptib, CDrawPortPtr pdp)
{
  if(INDEX(theApp.m_iTerrainEditMode)==TEM_HEIGHTMAP) ptib->tib_iIcon=20;
  else                                                ptib->tib_iIcon=19;
}

void UpdateBrushModeIcon(CTIButton *ptib, CDrawPortPtr pdp)
{
  INDEX iIcon;
  CTString strText;
...
void UpdateLayerVisibleFlag(CTIButton *ptib, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  CTerrainLayerPtr ptlLayer=GetLayer(iLayer);
  if(ptlLayer)
...
void UpdatePressure(CTIButton *ptib, CDrawPortPtr pdp)
{
...
void SetAsActiveLayer(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
...
void DisplayLayerTexture(INDEX iLayer)
{
  ...
  if(ptlLayer && ptlLayer->tl_ptdTexture!=NULL)
  {
    ...
    GetCursorPos(&pt);
    CTextureDataPtr ptd(ptlLayer->tl_ptdTexture);
    CTString strText1=ptd->GetName();
    CTString strText2=ptd->GetDescription();
    pDisplay->Initialize(pt.x, pt.y, ptd, strText1, strText2);
  }
}
...
void ApplyLayerTextureCommand(INDEX iSelectedItem)
{
  ...
  if(ptlLayer)
  {
    ...
    case 0: // "Create izohipse textutre"
    {
      ...
      CTextureDataPtr ptdGradient;
      try
      {
        ptdGradient=_pTextureStock_Obtain_t( fnGradient);
      ...
      _pTextureStock_Release(ptdGradient);
...
void DisplayLayerMask(INDEX iLayer)
{
  ...
  if(ptlLayer && ptlLayer->tl_ptdTexture!=NULL)
  {
    CTextureDataPtr ptd=ptlLayer->GetThumbnail(ptlLayer->tl_iMaskWidth-1,ptlLayer->tl_iMaskHeight-1);
...
void ApplyLayerMaskCommand(INDEX iSelectedItem)
{
  ...
  if(ptlLayer)
  {
    ...
    case 0: // "Fill mask"
    {
      ...
      {
        FLOAT3D vHit(0, 0, 0);
        EditTerrain({}, vHit, 1.0f, TE_FILL_LAYER_MASK);
      }
      break;
    }
    case 1: // "Clear mask"
    {
      ...
      {
        FLOAT3D vHit(0, 0, 0);
        EditTerrain({}, vHit, 1.0f, TE_CLEAR_LAYER_MASK);
      }
      break;
    }
    case 2: // "Import mask"
    ...
    {
      FLOAT3D vHit(0, 0, 0);
      EditTerrain({}, vHit, 1.0f, TE_LAYER_RND_NOISE);
      break;
    }
    case 5: // "View mask"
    {
...
void PickLayerColor(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  CTerrainLayerPtr ptlLayer=GetLayer(iLayer);
  if(!ptlLayer) return;
...
void NumericAlpha(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  CTerrainLayerPtr ptlLayer=GetLayer(iLayer);
  if(!ptlLayer) return;
...
void InvokeLayerOptions(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  if(iLayer==-1) return;
...
void PickSelectedLayerBcgColor(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
...
void InvokeLayerPopup(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  CTerrainPtr ptrTerrain=GetTerrain();
  if( !ptrTerrain) return;
...
void InvokeLayerTexturePopup(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  CTerrainPtr ptrTerrain = GetTerrain();
  if (!ptrTerrain) return;
...
void InvokeLayerMaskPopup(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  CTerrainPtr ptrTerrain = GetTerrain();
  if (!ptrTerrain) return;
...
CTString GetStretchInfo(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp, BOOL bLmb)
{
  CTString strInfo;
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
...
CTString GetEditedData(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp, BOOL bLmb)
{
  CTString strInfo;
...
void OnDropIntoLayerTexture(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp, CTFileName fnDropped)
{
  ...
  if(ptlLayer && ptrTerrain)
  {
    ...
    {
      fnOldTexture=CTextureDataPtr(ptlLayer->tl_ptdTexture)->GetName();
    }
...
void ToggleLayerVisibleFlag(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
  {
...
void ToggleFlag(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
  {
    ...
    if(ptlLayer)
    {
      BOOL bOldValue=*(BOOL*)((UBYTE*)ptlLayer.get_handle() + (ULONG)ptib->tib_pfData1);
      bOldValue=!bOldValue;
      *(BOOL*)((UBYTE*)ptlLayer.get_handle() + (ULONG)ptib->tib_pfData1) = bOldValue;
    }
  }
}
...
{
  CTerrainPtr ptrTerrain=GetTerrain();
  return ptrTerrain ? TRUE : FALSE;
}

CTString GetNormalizedPercentageInfo(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp, BOOL bLmb)
{
  CTString strInfo;
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
...
void ChangeLayerDistributionData(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp)
{
...
void ChangeData(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp)
{
...
void ChangeTextureRotation(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
  {
...
void ChangeTextureOffset(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
  {
...
void ChangeTextureStretch(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  if( iLayer!=-1)
  {
...
void InvokeBrushModeCombo(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  PointToScreenSpace(pt, pdp);
  CCustomComboWnd *pCombo=new CCustomComboWnd;
  INDEX iFlag=6;
...
void InvokePercentageCombo(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  PointToScreenSpace(pt, pdp);
  CCustomComboWnd *pCombo=new CCustomComboWnd;
...
void InvokeEditModeCombo(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  PointToScreenSpace(pt, pdp);
  CCustomComboWnd *pCombo=new CCustomComboWnd;
...
FLOAT GetMaxSliderPos(CDrawPortPtr pdp)
{
  CTerrainPtr ptrTerrain=GetTerrain();
  if( !ptrTerrain) return 0;
  FLOAT fMaxPos=ClampDn(ptrTerrain->tr_atlLayers.Count()*LAYER_HEIGHT-pdp->GetHeight(),PIX(0));
  return fMaxPos;
}

void OnSliderArrowUp(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  _fScrollLayers=ClampDn(_fScrollLayers-PIX(LAYER_HEIGHT),0.0f);
}

void OnSliderArrowDown(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  _fScrollLayers=ClampUp(_fScrollLayers+PIX(LAYER_HEIGHT),GetMaxSliderPos(pdp));
}

void RenderLayerColor(CTIButton *ptib, CDrawPortPtr pdp)
{
  INDEX iLayer=ptib->tib_iLayer;
  CTerrainLayerPtr ptlLayer=GetLayer(iLayer);
  if(!ptlLayer) return;
...
void RenderSlider(CTIButton *ptib, CDrawPortPtr pdp)
{
...
void OnSliderClick(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  FLOAT fMaxPos=GetMaxSliderPos(pdp);
  PointToIconSpace(pt, ptib);
  pt.y=ClampUp<INDEX>( pt.y, INDEX(ptib->tib_fdy-SLIDER_HOLDER_HEIGHT/2));
...
void DragSlider(CTIButton *ptib, FLOAT fdx, FLOAT fdy, CDrawPortPtr pdp/*=NULL*/)
{
  FLOAT fMaxPos=GetMaxSliderPos(pdp);
...
void RecalculateShadows(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  RecalculateShadows();
}

void DisplayHeightMap(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  PointToScreenSpace(pt, pdp);
...
void InvokeTerrainOptions(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  CTerrainPtr ptrTerrain=GetTerrain();
  if( !ptrTerrain) return;
...
void DisplayHeightMapWindow(CPoint pt)
{
  ...
  ii.ii_Width=pixW-1;
  ii.ii_Height=pixH-1;
  ii.ii_Picture=(UBYTE*)AllocMemory_((pixW-1)*(pixH-1)*sizeof(COLOR));
  COLOR *pcol=(COLOR *)ii.ii_Picture;
  ...
  try
  {
    ...
    CTFileName strHeightMap=CTString("Temp\\ViewHeightMap.tex");
    tdHeightMap.Save_t( strHeightMap);

    CTextureDataPtr ptd;
    ptd=_pTextureStock_Obtain_t( strHeightMap);
    ptd->Reload();
    CWndDisplayTexture *pDisplay=new CWndDisplayTexture;
...
void GenerateLayerDistribution(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  GenerateLayerDistribution(-1);
}

void InvokeImportExportCombo(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  CTerrainPtr ptrTerrain=GetTerrain();
  if( !ptrTerrain) return;
...
void InvokeTerrainBrushPalette( PIX pixX, PIX pixY)
{
  ...
  if( _pBrushPalette == NULL)
  {
    ...
    // initialize canvas for active texture button
    _pGfx_CreateWindowCanvas( _pBrushPalette->m_hWnd, _pBrushPalette->m_pViewPort,
...
void InvokeBrushPalette(CTIButton *ptib, CPoint pt, CDrawPortPtr pdp)
{
  PointToScreenSpace(pt, pdp);
...
int CTerrainInterface::OnToolHitTest( CPoint point, TOOLINFO* pTI ) const
{
  for (auto& ittib : dcButtons)
  {
    ...
    if( IsClicked(tib, point))
    {
      if( tib.tib_strToolTip!="" && static_cast<const char*>(tib.tib_strToolTip)[0]!='_')
      {
...
void CTerrainInterface::InitializeInterface(CDrawPortPtr pdp)
{
	try
  {
    DECLARE_CTFILENAME( fnTerrainEditIcons, "Textures\\Editor\\TerrainEditingIcons.tex");
  ...
  */
  ...
  if( ptrTerrain)
  {
    ...
    {
      CTerrainLayerPtr tlLayer=ptrTerrain->tr_atlLayers[iLayer];
