...
BOOL CConsoleSymbolsCombo::PreTranslateMessage(MSG* pMsg) 
{
  if(pMsg->message==WM_KEYDOWN && pMsg->wParam==VK_RETURN)
  {
    ...
    if( iSelectedSymbol != LB_ERR)
    {
      ...
      CTString strSelectedSymbol = static_cast<const char*>(CStringA(strSelectedSymbolW));
      ...
      ((CDlgConsole *)GetParent())->m_ctrlEditConsole.ReplaceSel( CString(static_cast<const char*>(strSelectedSymbol)), TRUE);
