...
#include <SeriousEngineCAPI/Base/Console.h>
...
{
  INDEX ctLines = _pConsole_con_ctLines();
  INDEX ctChars = _pConsole_con_ctCharsPerLine();
  // allocate new string with double line ends
  char *strNew = (char*)AllocMemory_(ctLines*(ctChars+2)+1);
  const char *strString = _pConsole_GetBuffer();
  char *pch = strNew;
  // convert '\n' to '\r''\n'
  while(*strString!=0) {
    if (*strString=='\n') {
...
BOOL CEditConsole::PreTranslateMessage(MSG* pMsg) 
{
  ...
  if(pMsg->message==WM_KEYDOWN && pMsg->wParam==VK_RETURN)
  {
    ...
    if( !bCtrl && (iCharOffset != -1) )
    {
      ...
      CTString strToExecute = static_cast<const char*>(CStringA(achrToExecute));
      CPrintF( ">%s\n", static_cast<const char*>(strToExecute));
      if( static_cast<const char*>(strToExecute)[strToExecute.Length()-1] != ';')
      {
        strToExecute += ";";
      }
      _pShell_Execute(strToExecute);
      // set new text for output window
      SetTextFromConsole();
      // remember input text into console input buffer
      CString sHistory;
      GetWindowText(sHistory);
      theApp.gam_strConsoleInputBuffer = static_cast<const char*>(CStringA(sHistory));
    }
